...
> 归档说明（2026-08-27）：项目已改为直接采集和训练抓角点任务。本文件仅保留历史配置与诊断证据；下方未完成的 20 次 approach-only 验收和 `M=30` 张开夹爪录制不再属于当前执行计划，也不是五相机实验的前置门槛。
...
以下清单和标准仅保存原实验设计，未完成项不再构成当前 TODO。
...
原通过标准：至少 20 次执行均有完整证据，且根据实测捕获区域阈值报告左右臂及
双臂进入捕获区域的比例。模型到达“大致角点附近”不视为通过。

## 已取消的历史人工真机门

已完成能在无人干预下安全完成的代码、单测、dry-run 和旧 MCAP 离线验证。原计划是
用 `M=30`、夹爪强制张开录制 7 条初筛 episode；该步骤现已取消。相关命令仅作为
历史复现实录保留，不应被当作当前 TODO。
